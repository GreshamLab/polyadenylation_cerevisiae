...
gtf <- read.table("../data/genome_gtf/Saccharomyces_cerevisiae.R64-1-1.CLEAN.gtf", 
                  ...
                  col.names = c("sequence", 
                                ...
                                "attribute"),
                  sep = '\t') %>% #import data
  mutate(attribute = str_replace(attribute, ";.*", ""), #fix gene names
         attribute = str_replace(attribute, "gene_id ", "")) %>% #fix gene names
...
#Pivot the gtf file
gtf <- gtf %>% 
  filter(! attribute %in% duplicated$attribute) %>%
  drop_na() %>%
...
peaks <- rawdata %>% 
  map(~.x %>%
        ...
        mutate(gene_length = gene_end - gene_start, #calculate gene length
               ...
               peak_summit = peak_start + relative_summit_position_to_peak_start, #calculate peak position on chromosome
