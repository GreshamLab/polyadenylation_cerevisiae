---
title: "Check peaks"
author: "Simone Zaghen"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: 
  html_document: 
    toc: true
    number_section: true
    toc_float: 
      collapsed: false
      smooth_scroll: true
---

# Load packages

```{r, warning=FALSE, message=FALSE}
library(tidyverse)
library(knitr)
library(kableExtra)

```

```{r }
no_hist <- read.delim("/scratch/sz4633/polyadenylation_cerevisiae/peaks_coverage_no_hist/RAPA_REP2_1", 
                      header = F, 
                      col.names = c("chromosome", 
                                    "peak_start",
                                    "peak_end",
                                    "peak_name",
                                    "score",
                                    "strand_macs",
                                    "Fold_Change_at_peak_summit",
                                    "log10pvalue_at_peak_summit",
                                    "log10qvalue_at_peak_summit",
                                    "relative_summit_position_to_peak_start",
                                    "chromosome_position",
                                    "gene_start",
                                    "gene_end",
                                    "gene_name",
                                    "tss",
                                    "strand",
                                    "features in B that overlapped the A interval",
                                    "bases in A with non-zero coverage from features in B",
                                    "length of the entry in A",
                                    "bases in A that had non-zero coverage from features in B"))

ggplot(no_hist, aes(x=features.in.B.that.overlapped.the.A.interval))+
  geom_density()+
  xlim(0,250000)

ggplot(no_hist, aes(x=bases.in.A.with.non.zero.coverage.from.features.in.B))+
  geom_density()

ggplot(no_hist, aes(x=length.of.the.entry.in.A))+
  geom_density()

ggplot(no_hist, aes(x=Fold_Change_at_peak_summit))+
  geom_density()

ggplot(no_hist, aes(x=log10pvalue_at_peak_summit))+
  geom_density()

```

# Import the data 

```{r, warning=FALSE, message=FALSE}
setwd("/scratch/sz4633/polyadenylation_cerevisiae/peaks_coverage_no_hist/")

#import peak files as list
peak_files <- list.files(pattern = "RAPA*")
rawdata <- lapply(peak_files, 
                     read.csv2,
                     sep = "\t",
                     dec = ".",
                     header = F,
                     col.names = c("chromosome",
                                   "peak_start",
                                   "peak_end",
                                   "peak_name",
                                   "score",
                                   "strand_macs",
                                   "Fold_Change_at_peak_summit",
                                   "log10pvalue_at_peak_summit",
                                   "log10qvalue_at_peak_summit",
                                   "relative_summit_position_to_peak_start",
                                   "chromosome_position",
                                   "gene_start",
                                   "gene_end",
                                   "gene_name",
                                   "tss",
                                   "strand",
                                   "features in B that overlapped the A interval",
                                   "bases in A with non-zero coverage from features in B",
                                   "length of the entry in A",
                                   "bases in A that had non-zero coverage from features in B"))


#assign names to peak files in list
peak_files <- str_remove(peak_files, "_intersect")
names(rawdata) <- peak_files
rm(peak_files)

```

Load GTF and wrangle 

```{r}
gtf <- read.table("../data/genome_gtf/Saccharomyces_cerevisiae.R64-1-1.CLEAN.gtf", 
                  header = FALSE,
                  col.names = c("sequence", 
                                "source", 
                                "feature", 
                                "start", 
                                "end", 
                                "score", 
                                "strand", 
                                "frame", 
                                "gene_name"),
                  sep = '\t') %>% #import data
  mutate(gene_name = str_replace(gene_name, ";.*", ""), #fix gene names
         gene_name = str_replace(gene_name, "gene_id ", "")) %>% #fix gene names
  filter(feature %in% c("transcript", "start_codon", "stop_codon")) %>%
  select(-c("sequence", "source", "score", "strand", "frame"))

```

Some genes have duplicated start codons that mess up the pivoting. Remove them

```{r}
duplicated <- gtf %>%
  group_by(gene_name, feature) %>%
  summarise(n = n(), .groups = "drop") %>%
  filter(n > 1L)
print(duplicated)

#Pivot the gtf file
gtf <- gtf %>% 
  filter(! gene_name %in% duplicated$gene_name) %>%
  drop_na() %>%
  pivot_wider(names_from = c("feature"), values_from = c("start", "end")) %>%
  select(-c("end_start_codon", "end_stop_codon"))

colnames(gtf) <- c("gene_name", 
                   "start_transcript",
                   "start_codon",
                   "stop_codon",
                   "end_transcript")

rm(duplicated)

```

Merge peaks_list dataframes with the gtf file. 

```{r}
peaks <- rawdata %>% 
  map(~.x %>%
        select(-c("chromosome_position",
                  "strand_macs",
                  "gene_start",
                  "gene_end",
                  "tss")) %>%
        merge(gtf,
              by.x = "gene_name",
              by.y = "gene_name")) #remove non important cols

```

# Calculate important statistics

- Transcript length
- abs_peak_summit: absolute peak position - peak position on chromosome
- Peak width: info needed to filter out peaks that are too broad
- Peak to transcript start
- Peak to transcript end
- Peak to start codon
- Peak to stop codon

```{r}
peaks <- peaks %>% 
  map(~.x %>%
        mutate(transcript_length = end_transcript - start_transcript,
               abs_peak_summit = peak_start + relative_summit_position_to_peak_start,
               peak_width = peak_end - peak_start, 
               peak_to_transcript_start =
                 ifelse(strand == "+", #calculation is different depending on strand
                        abs_peak_summit - start_transcript,
                        end_transcript - abs_peak_summit),
               peak_to_transcript_end = 
                 ifelse(strand == "+", 
                        end_transcript - abs_peak_summit,
                        abs_peak_summit - start_transcript),
               peak_to_start_codon =
                 ifelse(strand == "+",
                        abs_peak_summit - start_codon,
                        start_codon - abs_peak_summit),
               peak_to_stop_codon = 
                 ifelse(strand == "+",
                        stop_codon - abs_peak_summit,
                        abs_peak_summit - stop_codon)))

```

# Check distributions

## Distance to start/stop of transcript/codon

```{r, warning=FALSE,fig.height=10, fig.width=8}

peaks %>%
  bind_rows(.id = "sample") %>% #create one df
  select(c("gene_name",
           "peak_width",
           "peak_to_transcript_start",
           "peak_to_transcript_end",
           "peak_to_start_codon",
           "peak_to_stop_codon")) %>%
  pivot_longer(!gene_name, 
               names_to = "variable",
               values_to = "values") %>%
  ggplot(aes(x = values)) + 
  geom_histogram(bins = 1000) +
  xlim(-2000,4000) +
  facet_wrap(~variable,
             scales = "free_y",
             ncol = 2)

```

```{r, warning=FALSE,fig.height=6, fig.width=6}

peaks %>%
  bind_rows(.id = "sample") %>% #create one df
  select(c("gene_name",
           "Fold_Change_at_peak_summit",
           "log10pvalue_at_peak_summit",
           "log10qvalue_at_peak_summit"

           )) %>%
  pivot_longer(!gene_name, 
               names_to = "variable",
               values_to = "values") %>%
  ggplot(aes(x = values)) + 
  geom_histogram(bins = 1000) +
#  xlim(-2000,4000) +
  facet_wrap(~variable,
             scales = "free",
             nrow = 3)

```


## How many peaks are assigned to more than one gene

Peaks can be broad and can overlap with multiple genes. When this happens, a single peak (which has a unique peak name) will be assigned to multiple genes. For example peak_1 might be assigned to gene A and gene B. 

![Example from IGV of what i mean](/home/sz4633/polyadenylation_cerevisiae/igv_images/img_multiple_genes_per_peak.png)

Here we check how many peaks have been assigned to multiple genes. 

```{r }
peaks_assigned_to_genes <- data.frame()

for (i in 1:length(peaks)) {
  tmp <- as.data.frame(table(peaks[[i]]$peak_name))
  tmp <- as.data.frame(table(tmp$Freq))
  tmp <- cbind(tmp, names(peaks[i]))
  peaks_assigned_to_genes <- rbind(peaks_assigned_to_genes, tmp)
}

colnames(peaks_assigned_to_genes) <- c("genes_assigned_to_peak", 
                                       "number_of_peaks", 
                                       "sample")

peaks_assigned_to_genes <-peaks_assigned_to_genes %>%
  group_by(genes_assigned_to_peak) %>%
  summarise(number_of_peaks = sum(number_of_peaks))

kable(peaks_assigned_to_genes) %>%
  kable_styling(full_width = F, position = "left")

rm(peaks_assigned_to_genes, tmp, i)

```

## Check how many genes have more than one peak

One single gene might contain multiple peaks. This is what we are looking for as it is a sign of different 3'UTR.
![Example from IGV of what i mean](/home/sz4633/polyadenylation_cerevisiae/igv_images/img_multiple_peaks_per_gene.png)

```{r}
peaks_per_gene <- data.frame()

for (i in 1:length(peaks)) {
  tmp <- as.data.frame(table(peaks[[i]]$gene_name))
  tmp <- as.data.frame(table(tmp$Freq))
  tmp <- cbind(tmp, names(peaks[i]))
  peaks_per_gene <- rbind(peaks_per_gene, tmp)
}

colnames(peaks_per_gene) <- c("peaks_per_gene", "frequency", "sample")

peaks_per_gene <- peaks_per_gene %>%
        group_by(peaks_per_gene) %>%
        mutate(peaks_per_gene = as.numeric(peaks_per_gene)) %>%
        summarise(frequency = sum(frequency))

kable(peaks_per_gene) %>%
    kable_styling(full_width = F, position = "left")

rm(peaks_per_gene, tmp, i)

```

# Filter out some peaks

## Remove peaks whose summit is too close to the TSS

Some peaks are broad enough that one of their tails is very close to the TSS of a gene. These peaks are for the stop of the other gene, not for the start of a gene. However bedtools intersect assign that peak to both genes since the peak technically overlaps - here remove these types of peaks. 
![Example from IGV of what i mean](/home/sz4633/polyadenylation_cerevisiae/igv_images/peaks_close_to_tss.png)

Quantify how many of these instances we have:

```{r}
kable(sapply(peaks %>% 
               map(~.x %>%
                     filter(peak_to_transcript_start < 50)),
             nrow)) %>%
  kable_styling(full_width = F, position = "left")

```

Remove them

```{r}
peaks <- peaks %>% 
  map(~.x %>%
        filter(peak_to_transcript_start > 50))

```

## Remove peaks that are too broad

Peaks that have a width above the transcript length will be filtered out since they are too broad

Quantify how many of these instances we have:

```{r}
tmp1 <- as.data.frame(sapply(peaks %>% 
                 map(~.x %>%
                       filter(peak_width > 500)),
             nrow))

tmp2 <- as.data.frame(sapply(peaks %>% 
                 map(~.x %>%
                       filter(peak_width > transcript_length)),
             nrow))
tmp <- cbind(tmp1,tmp2) %>% rownames_to_column()
colnames(tmp) <- c("Sample", "Cutoff_500", "Cutoff_Transcript_Length")

kable(tmp) %>%
  kable_styling(full_width = F, position = "left")

rm(tmp, tmp1, tmp2)

```

Remove them 

```{r }
peaks <- peaks %>% 
  map(~.x %>%
        filter(peak_width < transcript_length))

```

## Remove genes with only one peak

Now that we filtered out most of the unwanted peaks, let's find the genes with more than one peak! 

```{r}
peaks <- peaks %>% 
  map(~.x %>%
        group_by(gene_name) %>% 
        filter(n()>1))

```

# Fish out most interesting genes

```{r}
peaks <- peaks %>% 
  map(~.x %>%
        group_by(gene_name) %>% 
        mutate(sd_peak_to_end = sd(peak_to_transcript_end))%>%
        arrange(desc(sd_peak_to_end)))

```

at some point report examples of genes with many peaks, like get some screenshots from IGV

# Export the peaks

```{r}
if (!dir.exists("/scratch/sz4633/polyadenylation_cerevisiae/filtered_peaks/"))
  {dir.create("/scratch/sz4633/polyadenylation_cerevisiae/filtered_peaks/")}

wd = ("/scratch/sz4633/polyadenylation_cerevisiae/filtered_peaks/")

saveRDS(peaks, file = paste0(wd,"my_data.rds"))

for (i in 1:length(peaks)) {
  write_delim(peaks[[i]][2:5],
              file = paste0(wd, names(peaks)[i], ".bed"), 
              col_names = F,
              delim="\t") 
}

```
